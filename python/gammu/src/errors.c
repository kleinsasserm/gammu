...
int gammu_create_errors(PyObject *d) {
    ...
    /* Help text */
    help_text = PyBytes_FromString("Generic class as parent for all gammu exceptions. This is never raised directly.");
    if (help_text == NULL)
        return 0;
	...
	for (error = ERR_NONE; error < ERR_LAST_VALUE; error++) {
        ...
        /* Help text */
        help_text = PyBytes_FromFormat("Exception corresponding to gammu error ERR_%s.\n"
        ...
        PyDict_SetItem(errornumber_list, id, PyBytes_FromString(errname));
        Py_DECREF(id);
    }
